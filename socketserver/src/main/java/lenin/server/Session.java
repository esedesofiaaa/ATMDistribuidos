...
/**
 * Session — Capas 1 y 2 del protocolo, lado servidor.
 *
 ...
 *
 * Reemplaza por completo a ObjectInputStream / ObjectOutputStream, que eran
 * serializacion nativa de Java y por tanto ilegibles para un cliente Python.
 */
public class Session {
   ...
   *         (byte -1) o si hubo un error de E/S. Devolver null es la senal
   *         de "no hay mas mensajes": quien llama debe terminar su bucle.
   */
  public String read() {
    if (this.input == null) {
      return null;
    ...
    try {
      while (true) {
        ...
        // Fin de stream: el otro extremo cerro la conexion. Se corta el bucle
        // de inmediato para no girar en vacio quemando CPU.
        if (byteRead == -1) {
          return null;
        }
        ...
        // Se ignora '\r' para tolerar clientes que envien CRLF (telnet, nc).
        if (byteRead == 13) {
          continue;
        }
  ...
  /** Une los campos con '|' para formar el mensaje. Ej: OK|Saldo: 500.00 */
  public static String encode(String... fields) {
    return String.join(SEPARATOR, fields);
  }

  /**
   * Parte el mensaje en sus campos. El primero es el comando o el estado.
   * Se escapa el pipe porque split() recibe una expresion regular, y el
   * limite -1 conserva los campos vacios finales.
   */
  ...
  /**
   * Cierra streams y socket. Cada cierre va aislado para que un fallo en uno
   * no impida los siguientes, y ninguna excepcion escapa del metodo.
   */
